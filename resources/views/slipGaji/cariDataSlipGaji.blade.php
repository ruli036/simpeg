                                ...
                                <span class="small">
                                   <Form action="{{url('search-slipGaji')}}" method="GET">
                                    ...
                                    <div class="form-group row ">                                     
                                      ...
                                      <span>Search</span>
                                          <input id="cari" type="text" value="{{$cari}}"  class=" form-control form-control-user @error('cari') is-invalid @enderror" name="cari"    placeholder="Name"  >
                                           @error('cari')
                                          <span class="invalid-feedback" role="alert">
                                              <strong>{{ $message }}</strong>
                                          </span>
                                    ...
                                    <tbody style="font-size: 15px">
                                          @foreach ($datas as $key => $data)
                                             ...
                                             <tr>
                                                ...
                                                <td>{{date('d F Y', strtotime($data->bulan))}}</td>
                                                  <td>{{$data->nik}}</td>
                                                  <td>{{$data->nama}}</td>
                                                  <td>
                                                      {{$data->divisi}} <br>
                                                  ...
                                                  <td>@currency($data->total_tunjangan)</td>
                                                  <td>@currency($data->uang_harian +$data->uang_lembur+$data->jam_tambahan+$data->jam_ganti+$data->lembur_kegiatan)</td>
                                                  <td>@currency($data->total_upah_lain )</td>
                                                  <td>(- @currency($data->potongan))</td>
                                                  <td>@currency($data->total + $data->total_upah_lain)</td>
                                                 <td class="text-center"> 
                                                    <a type="button" id="print" title="Download"  class="btn btn-primary btn btn-sm text-light" href="{{url('SlipGajiPdf',$data->id_slip)}}" target="_blank"  >
                                                        <i class="fas fa-cloud-download-alt"></i> 
                                                    </a>
                                                    <a type="button" id="print" title="Detail" class="btn btn-info btn btn-sm text-light" href="{{url('detailslipgaji',$data->id_slip)}}"    >
                                                    ...
                                                    <a type="button" title="Hapus" class="btn btn-danger btn btn-sm" data-toggle="modal" data-target="#exampleModalhapus{{$data->id_slip}}">
                                     ...
                                     <tfoot>
                                        <th colspan="9" style="font-size: 18px">Total Gaji Karyawan :</th>
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <form method="POST" action="{{url('/hapusbanyakdata')}}">
                    @csrf
                    @method('DELETE')
                    <input type="text" hidden name="_divisi">
                    <input type="text" hidden name="_thn">
                    <input type="text" hidden name="_bln">
  ...
  <div class="modal fade" id="exampleModalhapus{{$data->id_slip}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body"> Yakin Ingin Menghapus Slip Gaji <b class="text-uppercase"> {{$data->nama}}</b> Bulan <b class="text-uppercase"> {{date('d F Y', strtotime($data->bulan))}} </b></div>
        <div class="modal-footer">
            <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
