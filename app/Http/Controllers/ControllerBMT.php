    ...
    public function AjukanPinjaman(Request $request){
        try{
            ...
            if($request['tabungan'] == 2){
                ...
                if(str_replace(',', '',$request['nominal']) > 5000000){
                    return  redirect()->back()->with(['warning'=>'Maksimal Pengajuan Pinjaman Rp 5.000.000 ']);
                }else{
    ...
    public function exportLaporanBMT(){
        if(Auth::check() && ( Auth::user()->status == 7 || Auth::user()->status == 0)){
            try{
                ...
                $dataSetoran = SetoranBMT::select(DB::raw('sum(nominal_bmt) as total_bmt'),
                ...
                $dataPinjaman = PinjamanBmt::select(DB::raw('sum(nominal) as total_pinjaman'),
